...
class PrefabBuilder {
    ...
    process(scriptPath, prefabPath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const compressedUuid = UuidUtils_1.UuidUtils.compressUuid(scriptUuid);
        ...
        if (structMatch) {
            ...
            while ((am = arrowRegex.exec(structMatch[1])) !== null) {
                ...
                if (!props.has(propName)) {
                    // 尝试从 @property 提取类型
                    const typeRegex = new RegExp(`@property\\(([^)]+)\\)\\s*(?:public|private|protected)?\\s*${propName}\\s*[:=]`);
    ...
    syncPrefab(json, structure, properties, scriptUuid, prefabUuid) {
        ...
        // 2. 递归构建节点树
        const build = (nodeStruct, parentId, currentPath) => {
            ...
            // 检查常用组件：Button
            if (nodeStruct.name.includes("cc.Button") ||
            ...
            properties.forEach(({ path: pathOrAsset, type: propType }, propName) => {
                ...
                if (propType === "cc.Prefab") {
                    ...
                    if (assetUuid) {
                        ...
                        console.log(`[PrefabBuilder] 绑定预制体: ${propName} -> ${pathOrAsset} (${assetUuid.substring(0, 8)}...)`);
                    }
                    else {
                ...
                console.log(`[PrefabBuilder] 绑定属性: ${propName} -> 节点[${targetId}] 类型:${propType} => __id__:${bindValue.__id__}`);
            });
        }
    }
    /**
     ...
     */
    resolveNodeId(target, pathMap, nameMap) {
        // 1. 完整路径匹配
        ...
        if (nameMap.has(target))
            return nameMap.get(target);
        // 3. 路径尾部匹配 (如 "HUD/ScoreLabel" 匹配 "Root/HUD/ScoreLabel")
        for (const [fullPath, id] of pathMap) {
            if (fullPath.endsWith("/" + target) || fullPath.endsWith(target))
                return id;
        }
        return undefined;
    }
     ...
     */
    getTypedBindingValue(json, nodeId, propType) {
        // cc.Node 直接绑定节点
        if (propType === "cc.Node") {
    ...
    mountComponent(json, nodeId, compName, node) {
        // 动态查找组件 UUID，优先在特定目录查找
        const scriptUuid = this.findScriptUuid(compName);
        if (scriptUuid) {
            const compId = json.length;
            json.push({
                __type__: UuidUtils_1.UuidUtils.compressUuid(scriptUuid),
                _name: "",
                _objFlags: 0,
                node: { __id__: nodeId },
                _enabled: true,
